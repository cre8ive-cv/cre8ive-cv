...
[Features](#Features) • [Quick Start](#quick-start) • [Usage](#usage) • [Themes](#themes--colors) • [API](#api-reference) • [Docs](#documentation)
...
**Key Configuration Options** (`src/config/config.js`):
- Queue size limits (max 40 requests)
- PDF generation timeout (30s default)
- Memory thresholds for auto-restart
- Concurrency (2-4 workers)
- Payload size limits (1MB JSON, 1.5MB photo)
